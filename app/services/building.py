...
class BuildingService:
    ...
    async def get_buildings_from_area(
        ...
        if all(v is not None for v in [lat_min, lat_max, lng_min, lng_max]):
            buildings = await BuildingRepository.get_by_rectangle(
                session,
                lat_min,
                lat_max,
                lng_min,
        ...
        if lat is not None and lng is not None and radius is not None:
            buildings = [b for b in buildings
                         if _haversine(lat, lng, b.latitude, b.longitude) <= radius]

        if not buildings:
            raise HTTPException(
                status_code=404,
                detail='Здания с организациями c заданными параметрами не найдены'
            )
